...
public:
    ...
    EScan Feed(TArrayRef<const TCell> key, const TRow& row) override {
        ...
        if (CompareTypedCellVectors(rowCells.data(), LastIndexKey->GetCells().data(), IndexColumnTypeInfos.data(), IndexColumnTypeInfos.size()) == 0) {
    ...
    TAutoPtr<IDestructable> Finish(EStatus status) override {
        if (status != EStatus::Done && StatusCode == Ydb::StatusIds::SUCCESS) {
            StatusCode = Ydb::StatusIds::INTERNAL_ERROR;
        }
        SendResponse();
        return this;
