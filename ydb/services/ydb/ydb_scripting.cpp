    ...
    ADD_REQUEST(ExecuteYql, ExecuteYqlRequest, ExecuteYqlResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_,
            new TGrpcRequestOperationCall<ExecuteYqlRequest, ExecuteYqlResponse>
                (ctx, &DoExecuteYqlScript, TRequestAuxSettings{RLSWITCH(TRateLimiterMode::Ru), nullptr, TAuditModeFlags::DatabaseApi | TAuditModeFlags::DmlAudit | TAuditModeFlags::ModifyingOrCriticalApi}));
    })
    ...
    ADD_REQUEST(StreamExecuteYql, ExecuteYqlRequest, ExecuteYqlPartialResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_,
            new TGrpcRequestNoOperationCall<ExecuteYqlRequest, ExecuteYqlPartialResponse>
                (ctx, &DoStreamExecuteYqlScript, TRequestAuxSettings{RLSWITCH(TRateLimiterMode::Rps), nullptr, TAuditModeFlags::DatabaseApi | TAuditModeFlags::DmlAudit | TAuditModeFlags::ModifyingOrCriticalApi}));
    })
    ...
    ADD_REQUEST(ExplainYql, ExplainYqlRequest, ExplainYqlResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_,
            new TGrpcRequestOperationCall<ExplainYqlRequest, ExplainYqlResponse>
                (ctx, &DoExplainYqlScript, TRequestAuxSettings{RLSWITCH(TRateLimiterMode::Rps), nullptr, TAuditModeFlags::DatabaseApi}));
    })
#undef ADD_REQUEST
}
