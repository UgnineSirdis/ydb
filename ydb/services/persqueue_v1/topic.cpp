                    ...
                    [this](TIntrusivePtr<TStreamGRpcRequest::IContext> context) {
                        ActorSystem_->Send(GRpcRequestProxyId_, new NKikimr::NGRpcService::TEvStreamTopicWriteRequest(context,
                            TRequestAuxSettings{
                                ...
                                .AuditModeFlags = TAuditModeFlags::DatabaseApi | TAuditModeFlags::ModifyingOrCriticalApi,
...
#define ADD_REQUEST(NAME, SVC, IN, OUT, ACTION) \
    ...
    ADD_REQUEST(DropTopic, TopicService, DropTopicRequest, DropTopicResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvDropTopicRequest(ctx, &DoDropTopicRequest, TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .AuditModeFlags = TAuditModeFlags::DatabaseApi | TAuditModeFlags::ModifyingOrCriticalApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_DROPTOPIC}));
    })

    ADD_REQUEST(CreateTopic, TopicService, CreateTopicRequest, CreateTopicResponse, {
        auto clusterCfg = ClustersCfgProvider->GetCfg();
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvCreateTopicRequest(ctx, std::bind(DoCreateTopicRequest, _1, _2, clusterCfg), TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .AuditModeFlags = TAuditModeFlags::DatabaseApi | TAuditModeFlags::ModifyingOrCriticalApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_CREATETOPIC}));
    })

    ADD_REQUEST(AlterTopic, TopicService, AlterTopicRequest, AlterTopicResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvAlterTopicRequest(ctx, &DoAlterTopicRequest, TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .AuditModeFlags = TAuditModeFlags::DatabaseApi | TAuditModeFlags::ModifyingOrCriticalApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_ALTERTOPIC}));
    })

    ADD_REQUEST(DescribeTopic, TopicService, DescribeTopicRequest, DescribeTopicResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvDescribeTopicRequest(ctx, &DoDescribeTopicRequest, TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .AuditModeFlags = TAuditModeFlags::DatabaseApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_DESCRIBETOPIC}));
    })

    ADD_REQUEST(DescribeConsumer, TopicService, DescribeConsumerRequest, DescribeConsumerResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvDescribeConsumerRequest(ctx, &DoDescribeConsumerRequest, TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .AuditModeFlags = TAuditModeFlags::DatabaseApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_DESCRIBECONSUMER}));
    })

    ADD_REQUEST(DescribePartition, TopicService, DescribePartitionRequest, DescribePartitionResponse, {
        ActorSystem_->Send(GRpcRequestProxyId_, new TEvDescribePartitionRequest(ctx, &DoDescribePartitionRequest, TRequestAuxSettings{.RlMode = RLSWITCH(TRateLimiterMode::Rps), .CustomAttributeProcessor = YdsProcessAttr, .AuditModeFlags = TAuditModeFlags::DatabaseApi, .RequestType = NJaegerTracing::ERequestType::TOPIC_DESCRIBEPARTITION}));
    })
#undef ADD_REQUEST
...
#define ADD_REQUEST_LIMIT(NAME, CB, LIMIT_TYPE, REQUEST_TYPE, AUDIT_MODE_FLAGS)                               \
                ...
                ActorSystem_->Send(GRpcRequestProxyId_,                                                       \
                    new TGrpcRequestOperationCall<Ydb::Topic::NAME##Request, Ydb::Topic::NAME##Response>      \
                        (ctx, &CB, TRequestAuxSettings{                                                       \
                            ...
                            .AuditModeFlags = AUDIT_MODE_FLAGS,                                               \
    ...
    ADD_REQUEST_LIMIT(UpdateOffsetsInTransaction, DoUpdateOffsetsInTransaction, Ru, TOPIC_UPDATEOFFSETSINTRANSACTION, TAuditModeFlags::DatabaseApi | TAuditModeFlags::ModifyingOrCriticalApi)
